...
import json
from androconf import warning, error, CONF, enable_colors, \
...
def _PrintNote(note, tab=0):
    ...
    print_fct('\t' * tab + '%s# %s%s' % (note_color, note,
              normal_color) + '\n')
    ...
    if len(exceptions) > 0:
        ...
        for i in exceptions:
            CONF['PRINT_FCT']('\t%s%s%s\n' % (CONF['COLORS']['EXCEPTION'
                              ], i.show_buff(), CONF['COLORS']['NORMAL'
                              ]))
...
def _PrintXRef(tag, items):
    ...
    for i in items:
        print_fct('%s: %s %s %s %s\n' % (tag, i[0].get_class_name(),
                  i[0].get_name(), i[0].get_descriptor(), ' '.join('%x'
                  % j.get_idx() for j in i[1])))
...
def PrettyShow(m_a, basic_blocks, notes={}):
    ...
    for i in basic_blocks:
        ...
        for ins in instructions:
            ...
            print_fct('%s%-20s%s' % (instruction_name_color,
                      ins.get_name(), normal_color))

            operands = ins.get_operands()
            print_fct('%s'
                      % ', '.join(m_a.get_vm().colorize_operands(operands,
                      colors)))
            ...
            if ins == instructions[-1] and i.childs:
                ...
                if (op_value == 0x2b or op_value == 0x2c) \
                    ...
                    print_fct(' '.join('%d:%s' % (values[j], i.childs[j
                              + 1][2].get_name()) for j in range(0,
                              len(i.childs) - 1)) + ' ]%s'
                              % normal_color)
                else:
                    if len(i.childs) == 2:
                        ...
                        print_fct(' '.join('%s' % c[2].get_name()
                                  for c in i.childs[1:]) + ' ]%s'
                                  % normal_color)
                    else:
                        print_fct('%s[ ' % branch_color + ' '.join('%s'
                                  % c[2].get_name() for c in i.childs)
                                  + ' ]%s' % normal_color)
...
def method2json_undirect(mx):
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockChild in DVMBasicMethodBlock.childs:
            ...
            cblock['Edge'
                   ].append(DVMBasicMethodBlockChild[-1].get_name())
...
def method2json_direct(mx):
    ...
    l = []
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        for (index, DVMBasicMethodBlockChild) in \
            enumerate(DVMBasicMethodBlock.childs):
            if DVMBasicMethodBlock.get_name() \
                == DVMBasicMethodBlockChild[-1].get_name():

                preblock = TmpBlock(DVMBasicMethodBlock.get_name()
                                    + '-pre')

                cnblock = {}
                cnblock['BasicBlockId'] = \
                    DVMBasicMethodBlock.get_name() + '-pre'
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockChild in DVMBasicMethodBlock.childs:
            ...
            if DVMBasicMethodBlock.get_name() in hooks:
                if DVMBasicMethodBlockChild[-1] \
                    in hooks[DVMBasicMethodBlock.get_name()]:
                    ok = True

                    cblock['Edge'
                           ].append(hooks[DVMBasicMethodBlock.get_name()][0].get_name())
