...
class TaintedVariables(object):
    ...
    def get_strings_by_method(self, method):
        ...
        try:
            for i in \
                self.__cache_string_by_method[method.get_method_idx()]:
                z[i] = []
    ...
    def get_fields_by_method(self, method):
        ...
        try:
            for i in \
                self.__cache_field_by_method[method.get_method_idx()]:
                z[i] = []
                for j in i.get_paths():
                    if method.get_method_idx() == j[1]:
                        z[i].append(j[0])
...
TAINTED_PACKAGE = {TAINTED_PACKAGE_CREATE: 'C',
...
class TaintedPackage(object):
    ...
    def get_method(self, name, descriptor):
        l = []
        for path in self.paths[TAINTED_PACKAGE_CALL]:
            if path.get_name() == name and path.get_descriptor() \
                == descriptor:
                l.append(path)
        return l
        ...
        for _type in self.paths:
            ...
            if _type == TAINTED_PACKAGE_CALL:
                for path in self.paths[_type]:
                    print '\t\t => %s <-- %x in %s' \
                        % (path.get_dst(cm), path.get_idx(),
                           path.get_src(cm))
            else:
                for path in self.paths[_type]:
                    print '\t\t => %x in %s' % (path.get_idx(),
                            path.get_src(cm))
...
def show_ReflectionCode(dx):
    ...
    paths = \
        dx.get_tainted_packages().search_methods('Ljava/lang/reflect/Method;'
            , '.', '.')
    show_Paths(dx.get_vm(), paths)
...
class TaintedPackages(object):
    ...
    def get_update_packages_from_package(self, package):
        ...
        for (m, _) in self.get_packages():
            ...
            for j in paths:
                ...
                if src_class_name == package and dst_class_name \
                    in classes:
                    l.append(j)
        return l
    ...
    def get_external_packages(self):
        ...
        for (m, _) in self.get_packages():
            ...
            for j in paths:
                ...
                (dst_class_name, _, _) = \
                    j.get_dst(self.__vm.get_class_manager())
                if src_class_name in classes and dst_class_name \
                    not in classes:
                    if j.get_access_flag() == TAINTED_PACKAGE_CALL:
                        l.append(j)
        return l
...
TAGS_ANDROID = {
    ...
    TAG_ANDROID.ACCESSIBILITYSERVICE: [0,
...
class ExceptionAnalysis(object):
    ...
    def get(self):
        ...
        for i in self.exceptions:
            d['list'].append({'name': i[0], 'idx': i[1],
                             'bb': i[2].get_name()})
...
class MethodAnalysis(object):
    ...
    def create_tags(self):
      ...
      """
        ...
        self.tags = Tags()
        for i in \
            self.tainted.get_tainted_packages().get_packages_by_method(self.method):
...
class VMAnalysis(object):
    ...
    def get_permissions_method(self, method):
        permissions_f = \
            self.get_tainted_packages().get_permissions_method(method)
        permissions_v = \
            self.get_tainted_variables().get_permissions_method(method)
