...
class Signature(object):
    ...
    def __init__(self, vmx):
        ...
        self.levels = {  # Classical method signature with basic blocks, strings, fields, packages
                         # strings
                         # exceptions
                         # fill array data
            'L0': {
                ...
                1: ('_get_strings_pa', '_get_fields_a',
                    '_get_packages_a'),
                2: ('_get_strings_a', '_get_fields_a',
                    '_get_packages_pa_1'),
                3: ('_get_strings_a', '_get_fields_a',
                    '_get_packages_pa_2'),
                },
            'L1': ['_get_strings_a1'],
            'L2': ['_get_exceptions'],
    ...
    def _get_exceptions(self, analysis_method):
        ...
        for handler_catch in handler_catch_list.get_list():
            for handler in handler_catch.get_handlers():
                buff += \
                    analysis_method.get_vm().get_cm_type(handler.get_type_idx())
        return buff
            ...
            for path in packages_method[m]:
                ...
                if path.get_access_flag() == 1:
                    (dst_class_name, dst_method_name, dst_descriptor) = \
                        path.get_dst(analysis_method.get_vm().get_class_manager())
