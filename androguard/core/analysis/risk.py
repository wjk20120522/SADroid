...
from androguard.core.androconf import error, warning, debug, set_debug, \
    get_debug
...
def create_system_risk():
    ...
    input_Dangerous_Risk = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_Money_Risk = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_Privacy_Risk = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    ...
    input_Dangerous_Risk.adjectives[LOW_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (8.0, 1.0), (12.0, 0.0)]))
    input_Dangerous_Risk.adjectives[AVERAGE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(8.0,
                                  0.0), (50.0, 1.0), (60.0, 0.0)]))
    input_Dangerous_Risk.adjectives[HIGH_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(50.0,
                                  0.0), (85.0, 1.0), (95.0, 0.0)]))
    input_Dangerous_Risk.adjectives[UNACCEPTABLE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(85.0,
                                  0.0), (100.0, 1.0)]))

        # Money Risk

    system.variables['input_Money_Risk'] = input_Money_Risk
    input_Money_Risk.adjectives[LOW_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (2.0, 1.0), (3.0, 0.0)]))
    input_Money_Risk.adjectives[UNACCEPTABLE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(4.0,
                                  0.0), (5.0, 1.0), (30.0, 1.0)]))

        # Privacy Risk

    system.variables['input_Privacy_Risk'] = input_Privacy_Risk
    input_Privacy_Risk.adjectives[LOW_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (6.0, 1.0), (10.0, 0.0)]))
    input_Privacy_Risk.adjectives[HIGH_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(6.0,
                                  0.0), (10.0, 1.0), (20.0, 0.0)]))
    input_Privacy_Risk.adjectives[UNACCEPTABLE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(15.0,
                                  0.0), (20.0, 1.0), (30.0, 1.0)]))

        # Binary Risk

    system.variables['input_Binary_Risk'] = input_Binary_Risk
    input_Binary_Risk.adjectives[LOW_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (6.0, 1.0), (10.0, 0.0)]))
    input_Binary_Risk.adjectives[AVERAGE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(6.0,
                                  0.0), (10.0, 1.0), (15.0, 0.0)]))
    input_Binary_Risk.adjectives[HIGH_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(10.0,
                                  0.0), (20.0, 1.0), (24.0, 0.0)]))
    input_Binary_Risk.adjectives[UNACCEPTABLE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(23.0,
                                  0.0), (30.0, 1.0), (40.0, 1.0)]))
    ...
    input_Internet_Risk.adjectives[HIGH_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(1.0,
                                  0.0), (5.0, 1.0), (30.0, 1.0)]))

        # Dynamic Risk

    system.variables['input_Dynamic_Risk'] = input_Dynamic_Risk
    input_Dynamic_Risk.adjectives[LOW_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (2.0, 1.0), (3.0, 0.0)]))
    input_Dynamic_Risk.adjectives[UNACCEPTABLE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(4.0,
                                  0.0), (5.0, 1.0), (50.0, 1.0)]))

    # Output variables

    output_malware_risk = \
        fuzzy.OutputVariable.OutputVariable(defuzzify=fuzzy.defuzzify.COGS.COGS(),
            description='malware risk', min=0.0, max=100.0)
    ...
    output_malware_risk.adjectives[NULL_MALWARE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(0.0))
    output_malware_risk.adjectives[AVERAGE_MALWARE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(30.0))
    output_malware_risk.adjectives[HIGH_MALWARE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(60.0))
    output_malware_risk.adjectives[UNACCEPTABLE_MALWARE_RISK] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(100.0))
    ...
    add_system_rule(system, 'r0',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[NULL_MALWARE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Dynamic_Risk'
                    ].adjectives[LOW_RISK])))

    add_system_rule(system, 'r0a',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
    ...
    add_system_rule(system, 'r1',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[NULL_MALWARE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Money_Risk'
                    ].adjectives[LOW_RISK])))

    add_system_rule(system, 'r1a',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[UNACCEPTABLE_MALWARE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Money_Risk'
                    ].adjectives[UNACCEPTABLE_RISK])))
    ...
    add_system_rule(system, 'r3',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[AVERAGE_MALWARE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Binary_Risk'
                    ].adjectives[AVERAGE_RISK])))

    add_system_rule(system, 'r3a',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[HIGH_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Binary_Risk'
                    ].adjectives[HIGH_RISK])))

    add_system_rule(system, 'r3b',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[UNACCEPTABLE_MALWARE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Binary_Risk'
                    ].adjectives[UNACCEPTABLE_RISK])))
    ...
    add_system_rule(system, 'r5',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[HIGH_MALWARE_RISK]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Privacy_Risk'
                    ].adjectives[LOW_RISK]),
                    fuzzy.operator.Input.Input(system.variables['input_Internet_Risk'
                    ].adjectives[HIGH_RISK]))))

    add_system_rule(system, 'r5a',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[UNACCEPTABLE_MALWARE_RISK]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Privacy_Risk'
    ...
    add_system_rule(system, 'r6',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[HIGH_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Dangerous_Risk'
                    ].adjectives[HIGH_RISK])))

    add_system_rule(system, 'r6a',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_malware_risk'
                    ].adjectives[UNACCEPTABLE_RISK]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Dangerous_Risk'
                    ].adjectives[UNACCEPTABLE_RISK])))
...
def create_system_method_score():
    ...
    input_Match_MS = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_AndroidEntropy_MS = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_JavaEntropy_MS = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_Permissions_MS = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    input_Similarity_MS = \
        fuzzy.InputVariable.InputVariable(fuzzify=fuzzy.fuzzify.Plain.Plain())
    ...
    input_Length_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (50.0, 1.0), (100.0, 0.0)]))
    input_Length_MS.adjectives[AVERAGE_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(50.0,
                                  0.0), (100.0, 1.0), (150.0, 1.0),
                                  (300.0, 0.0)]))
    input_Length_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(150.0,
                                  0.0), (200.0, 1.0), (300.0, 1.0),
                                  (400.0, 0.0)]))
    input_Length_MS.adjectives[PERFECT_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(350.0,
                                  0.0), (400.0, 1.0), (500.0, 1.0)]))

        # Match

    system.variables['input_Match_MS'] = input_Match_MS
    input_Match_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (20.0, 1.0), (50.0, 0.0)]))
    input_Match_MS.adjectives[AVERAGE_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(40.0,
                                  0.0), (45.0, 1.0), (60.0, 1.0),
                                  (80.0, 0.0)]))
    input_Match_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(75.0,
                                  0.0), (90.0, 1.0), (98.0, 1.0),
                                  (99.0, 0.0)]))
    input_Match_MS.adjectives[PERFECT_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(98.0,
                                  0.0), (100.0, 1.0)]))
    ...
    system.variables['input_AndroidEntropy_MS'] = \
        input_AndroidEntropy_MS
    input_AndroidEntropy_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (2.0, 1.0), (4.0, 0.0)]))
    input_AndroidEntropy_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(3.0,
                                  0.0), (4.0, 1.0), (30.0, 1.0)]))

        # Java Entropy

    system.variables['input_JavaEntropy_MS'] = input_JavaEntropy_MS
    input_JavaEntropy_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (2.0, 1.0), (4.0, 0.0)]))
    input_JavaEntropy_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(3.0,
                                  0.0), (4.0, 1.0), (30.0, 1.0)]))
    ...
    input_Permissions_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (3.0, 1.0), (4.0, 0.0)]))
    input_Permissions_MS.adjectives[AVERAGE_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(3.0,
                                  0.0), (4.0, 1.0), (8.0, 1.0), (9.0,
                                  0.0)]))
    input_Permissions_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(8.0,
                                  0.0), (10.0, 1.0), (12.0, 1.0),
                                  (13.0, 0.0)]))
    input_Permissions_MS.adjectives[PERFECT_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(12.0,
                                  0.0), (13.0, 1.0), (20.0, 1.0)]))

        # Similarity Match

    system.variables['input_Similarity_MS'] = input_Similarity_MS
    input_Similarity_MS.adjectives[HIGH_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.0,
                                  1.0), (0.1, 1.0), (0.3, 0.0)]))
    input_Similarity_MS.adjectives[LOW_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Polygon.Polygon([(0.3,
                                  0.0), (0.35, 1.0), (0.4, 1.0)]))

    # Output variables

    output_method_score = \
        fuzzy.OutputVariable.OutputVariable(defuzzify=fuzzy.defuzzify.COGS.COGS(),
            description='method score', min=0.0, max=100.0)
    output_method_score.adjectives[NULL_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(0.0))
    output_method_score.adjectives[AVERAGE_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(50.0))
    output_method_score.adjectives[HIGH_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(80.0))
    output_method_score.adjectives[PERFECT_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(100.0))

    system.variables['output_method_score'] = output_method_score

    add_system_rule(system, 'android entropy null',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_AndroidEntropy_MS'
                    ].adjectives[LOW_SCORE])))

    add_system_rule(system, 'java entropy null',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_JavaEntropy_MS'
                    ].adjectives[LOW_SCORE])))

    add_system_rule(system, 'permissions null',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Permissions_MS'
                    ].adjectives[LOW_SCORE])))

    add_system_rule(system, 'permissions average',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[AVERAGE_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Permissions_MS'
                    ].adjectives[AVERAGE_SCORE])))

    add_system_rule(system, 'permissions high',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[HIGH_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Permissions_MS'
                    ].adjectives[HIGH_SCORE])))

    add_system_rule(system, 'permissions perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[PERFECT_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Permissions_MS'
                    ].adjectives[PERFECT_SCORE])))

    add_system_rule(system, 'similarity low',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_Similarity_MS'
                    ].adjectives[LOW_SCORE])))

    add_system_rule(system, 'length match perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[PERFECT_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
                    ].adjectives[PERFECT_SCORE]),
                    fuzzy.operator.Input.Input(system.variables['input_Match_MS'
                    ].adjectives[PERFECT_SCORE]))))

    add_system_rule(system, 'length match null',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
    ...
    add_system_rule(system, 'length AndroidEntropy perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[HIGH_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
                    ].adjectives[PERFECT_SCORE]),
                    fuzzy.operator.Input.Input(system.variables['input_AndroidEntropy_MS'
                    ].adjectives[HIGH_SCORE]))))

    add_system_rule(system, 'length JavaEntropy perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[HIGH_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
                    ].adjectives[PERFECT_SCORE]),
                    fuzzy.operator.Input.Input(system.variables['input_JavaEntropy_MS'
                    ].adjectives[HIGH_SCORE]))))

    add_system_rule(system, 'length similarity perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[PERFECT_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
                    ].adjectives[PERFECT_SCORE]),
                    fuzzy.operator.Input.Input(system.variables['input_Similarity_MS'
                    ].adjectives[HIGH_SCORE]))))

    add_system_rule(system, 'length similarity average',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_score'
                    ].adjectives[HIGH_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
...
def create_system_method_one_score():
    ...
    output_method_score = \
        fuzzy.OutputVariable.OutputVariable(defuzzify=fuzzy.defuzzify.COGS.COGS(),
            description='method one score', min=0.0, max=100.0)
    output_method_score.adjectives[NULL_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(0.0))
    output_method_score.adjectives[AVERAGE_METHOD_SCORE] = \
        fuzzy.Adjective.Adjective(fuzzy.set.Singleton.Singleton(50.0))
    ...
    add_system_rule(system, 'android entropy null',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_one_score'
                    ].adjectives[NULL_METHOD_SCORE]],
                    operator=fuzzy.operator.Input.Input(system.variables['input_AndroidEntropy_MS'
                    ].adjectives[LOW_SCORE])))
    ...
    add_system_rule(system, 'length permissions perfect',
                    fuzzy.Rule.Rule(adjective=[system.variables['output_method_one_score'
                    ].adjectives[PERFECT_METHOD_SCORE]],
                    operator=fuzzy.operator.Compound.Compound(fuzzy.norm.Min.Min(),
                    fuzzy.operator.Input.Input(system.variables['input_Length_MS'
                    ].adjectives[PERFECT_SCORE]),
                    fuzzy.operator.Input.Input(system.variables['input_Permissions_MS'
                    ].adjectives[PERFECT_SCORE]))))
...
def export_system(system, directory):
    ...
    d.overscan = 0
    in_vars = [name for (name, var) in system.variables.items()
               if isinstance(var, fuzzy.InputVariable.InputVariable)]
    out_vars = [name for (name, var) in system.variables.items()
                if isinstance(var, fuzzy.OutputVariable.OutputVariable)]

    if len(in_vars) == 2 \
        and not (isinstance(system.variables[in_vars[0]].fuzzify,
                 fuzzy.fuzzify.Dict.Dict)
                 or isinstance(system.variables[in_vars[1]].fuzzify,
                 fuzzy.fuzzify.Dict.Dict)):
        for out_var in out_vars:
            args = []
            if isinstance(system.variables[out_var].defuzzify,
                          fuzzy.defuzzify.Dict.Dict):
                for adj in system.variables[out_var].adjectives:
                    d.create3DPlot_adjective(
                        system,
                        in_vars[0],
                        in_vars[1],
                        out_var,
    ...
    def test(self):
      ...
      # #########################
        ...
        for method in vm.get_methods():
            ...
            score_order_sign[method] = \
                self.get_method_score(method.get_length(),
                    libelsign.entropy(vmx.get_method_signature(method,
                    'L4', {'L4': {'arguments': ['Landroid'
                    ]}}).get_string()),
                    libelsign.entropy(vmx.get_method_signature(method,
                    'L4', {'L4': {'arguments': ['Ljava'
                    ]}}).get_string()), map(lambda perm: (perm,
                    DVM_PERMISSIONS['MANIFEST_PERMISSION'][perm]),
                    vmx.get_permissions_method(method)))

        for v in sorted(score_order_sign, key=lambda x: \
                        score_order_sign[x], reverse=True):
...
#            print permission, GENERAL_PERMISSIONS_RISK[ list_details_permissions[ i ][0] ]
            ...
            risk_type = \
                GENERAL_PERMISSIONS_RISK[list_details_permissions[i][0]]
        ...
        for (m, _) in vmx.tainted_packages.get_packages():
            if m.get_name() == 'Ldalvik/system/DexClassLoader;':
                for path in m.get_paths():
                    if path.get_access_flag() \
                        == analysis.TAINTED_PACKAGE_CREATE:
    ...
    def get_method_score(
        ...
        for i in permissions:
            val_permissions += \
                RISK_VALUES[GENERAL_PERMISSIONS_RISK[i[1][0]]]
        ...
        self.system_method_risk.calculate(input=input_val,
...
class RedFlags(object):
    ...
    def __init__(self):
        self.flags = {'APK': {  # presence of shared libraries (ELF)
                                # presence of executables (ELF)
                                # presence of dex files
                                # presence of APK files
                                # presence of zip files
                                # presence of shell scripts
            'SHARED LIBRARIES': 0,
            'EXECUTABLE': 0,
            'DEX': 0,
            'APK': 0,
            'ZIP': 0,
            'SHELL_SCRIPT': 0,
            }, 'PERM': {  # presence of permissions which can manipulate sms
                          # presence of permissions which can perform a call
                          # presence of permissions which can manipulate your location
                          # presence of permissions which can result to a payement
                          # presence of permissions which can access to internet
                          # presence of permissions which can access to private information
            ...
            'SIGNATURE': 0,
            'SIGNATUREORSYSTEM': 0,
            }, 'DEX': {  # presence of the reflection API
                         # presence of loading a shared library
                         # presence of loading dynamically a new dex file
                         # presence of crypto functions
            'REFLECTION': 0,
            'NATIVE': 0,
            'DYNAMIC': 0,
            'CRYPTO': 0,
    ...
    def analyze_apk(self, a, flags):
        ...
        apk_file = [re.compile('Android application package file'),
                    'APK']
        dex_file = [re.compile('Dalvik dex file version 035'), 'DEX',
                    re.compile('^classes.dex$')]
        script_file = [re.compile('script text executable'),
                       'SHELL_SCRIPT']
