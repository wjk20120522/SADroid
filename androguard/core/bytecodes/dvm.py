...
class HeaderItem(object):
    ...
    def __init__(self, buff, cm):
        ...
        self.signature = unpack('=20s', buff.read(0x14))[0]
    ...
    def get_obj(self):
        ...
        return pack('=Q', self.magic) + pack('=i', self.checksum) \
            ...
            + pack('=I', self.link_off) + pack('=I', self.map_off) \
            + pack('=I', self.string_ids_size) + pack('=I',
                self.string_ids_off) + pack('=I', self.type_ids_size) \
            + pack('=I', self.type_ids_off) + pack('=I',
                self.proto_ids_size) + pack('=I', self.proto_ids_off) \
            + pack('=I', self.field_ids_size) + pack('=I',
                self.field_ids_off) + pack('=I', self.method_ids_size) \
            + pack('=I', self.method_ids_off) + pack('=I',
                self.class_defs_size) + pack('=I', self.class_defs_off) \
            + pack('=I', self.data_size) + pack('=I', self.data_off)
...
class AnnotationOffItem(object):
    ...
    def get_obj(self):
        if self.annotation_off != 0:
            self.annotation_off = \
                self.__CM.get_obj_by_offset(self.annotation_off).get_off()
...
class AnnotationSetItem(object):
    ...
    def get_raw(self):
        return self.get_obj() + ''.join(i.get_raw() for i in
                self.annotation_off_item)
    ...
    def get_obj(self):
        if self.annotations_off != 0:
            self.annotations_off = \
                self.__CM.get_obj_by_offset(self.annotations_off).get_off()
...
class AnnotationSetRefList(object):
    ...
    def get_raw(self):
        return pack('=I', self.size) + ''.join(i.get_raw() for i in
                self.list)
...
class FieldAnnotation(object):
    ...
    def get_obj(self):
        ...
        return pack('=I', self.field_idx) + pack('=I',
                self.annotations_off)
...
class MethodAnnotation(object):
    ...
    def get_obj(self):
        ...
        return pack('=I', self.method_idx) + pack('=I',
                self.annotations_off)
...
class AnnotationsDirectoryItem(object):
    ...
    def __init__(self, buff, cm):
        ...
        for i in xrange(0, self.annotated_parameters_size):
            self.parameter_annotations.append(ParameterAnnotation(buff,
                    cm))
    ...
    def get_obj(self):
        if self.class_annotations_off != 0:
            self.class_annotations_off = \
                self.__CM.get_obj_by_offset(self.class_annotations_off).get_off()

        return pack('=I', self.class_annotations_off) + pack('=I',
                self.annotated_fields_size) + pack('=I',
                self.annotated_methods_size) + pack('=I',
                self.annotated_parameters_size)

    def get_raw(self):
        return self.get_obj() + ''.join(i.get_raw() for i in
                self.field_annotations) + ''.join(i.get_raw() for i in
                self.method_annotations) + ''.join(i.get_raw() for i in
                self.parameter_annotations)
...
DBG_END_SEQUENCE = 0  #    (none)  terminates a debug info sequence for a code_item
DBG_ADVANCE_PC = 1  #     uleb128 addr_diff       addr_diff: amount to add to address register    advances the address register without emitting a positions entry
DBG_ADVANCE_LINE = 2  #    sleb128 line_diff       line_diff: amount to change line register by    advances the line register without emitting a positions entry
DBG_START_LOCAL = 3  #   uleb128 register_num
...
DBG_START_LOCAL_EXTENDED = 4  #   uleb128 register_num uleb128p1 name_idx uleb128p1 type_idx uleb128p1 sig_idx
...
DBG_SET_EPILOGUE_BEGIN = 8  #    (none)  sets the epilogue_begin state machine register, indicating that the next position entry that is added should be considered the beginning
...
class DebugInfoItem(object):
        ...
        while bcode.get_op_value() != DBG_END_SEQUENCE:
            ...
            if bcode_value == DBG_ADVANCE_PC:
                ...
                bcode.add(readuleb128p1(buff), 'u1')
                   ...
                   # bcode_value >= DBG_Special_Opcodes_BEGIN and bcode_value <= DBG_Special_Opcodes_END:
    ...
    def get_raw(self):
        return [bytecode.Buff(self.__offset,
                writeuleb128(self.line_start)
                + writeuleb128(self.parameters_size)
                + ''.join(writeuleb128(i) for i in
                self.parameter_names) + ''.join(i.get_raw() for i in
                self.bytecodes))]
...
VALUE_BYTE = 0  # (none; must be 0)      ubyte[1]         signed one-byte integer value
VALUE_SHORT = 2  # size - 1 (0..1)  ubyte[size]    signed two-byte integer value, sign-extended
VALUE_CHAR = 3  # size - 1 (0..1)  ubyte[size]    unsigned two-byte integer value, zero-extended
VALUE_INT = 4  # size - 1 (0..3)  ubyte[size]    signed four-byte integer value, sign-extended
VALUE_LONG = 6  # size - 1 (0..7)  ubyte[size]    signed eight-byte integer value, sign-extended
...
class EncodedValue(object):
    ...
    def get_raw(self):
        if self.raw_value == None:
            return pack('=B', self.val) \
                + bytecode.object_to_str(self.value)
        else:
            return pack('=B', self.val) \
                + bytecode.object_to_str(self.raw_value)

    def get_length(self):
        if self.raw_value == None:
            return len(pack('=B', self.val)) \
                + len(bytecode.object_to_str(self.value))
        else:
            return len(pack('=B', self.val)) \
                + len(bytecode.object_to_str(self.raw_value))
...
class EncodedAnnotation(object):
    ...
    def get_raw(self):
        return writeuleb128(self.type_idx) + writeuleb128(self.size) \
            + ''.join(i.get_raw() for i in self.elements)

    def get_length(self):
        length = len(writeuleb128(self.type_idx)
                     + writeuleb128(self.size))
    ...
    for _ in xrange(length):
        ...
        if value in (0, 1, 2, 3, 4, 5, 6, 0x07,):
            ...
            if value != 0 and value < 0x80:
                warning('at offset %x: utf8 should have been represented with one byte encoding'
                         % buff.get_idx())
            chars.append(unichr(value))
        elif value == 0x0e:
            second_char = ord(buff.read(1))
            if second_char & 0xc0 != 0x80:
                warning('bad utf8 byte %x at offset %x' % (second_char,
                        buff.get_idx()))
            third_char = ord(buff.read(1))
            if third_char & 0xc0 != 0x80:
                warning('bad utf8 byte %x at offset %x' % (third_char,
                        buff.get_idx()))
            value = (first_char & 15) << 0x0c | (second_char & 0x3f) \
                << 6 | third_char & 0x3f
            if value < 0x800:
                warning('at offset %x: utf8 should have been represented with two-byte encoding'
                         % buff.get_idx())
            chars.append(unichr(value))
        else:
...
class ProtoIdItem(object):
    ...
    def get_obj(self):
        if self.parameters_off != 0:
            self.parameters_off = \
                self.__CM.get_obj_by_offset(self.parameters_off).get_off()

        return pack('=I', self.shorty_idx) + pack('=I',
                self.return_type_idx) + pack('=I', self.parameters_off)
...
class FieldIdItem(object):
    ...
    def show(self):
        ...
        bytecode._PrintDefault('class_idx=%d type_idx=%d name_idx=%d\n'
                               % (self.class_idx, self.type_idx,
                               self.name_idx))
        bytecode._PrintDefault('class_idx_value=%s type_idx_value=%s name_idx_value=%s\n'
                                % (self.class_idx_value,
                               self.type_idx_value,
                               self.name_idx_value))

    def get_obj(self):
        return pack('=H', self.class_idx) + pack('=H', self.type_idx) \
...
class MethodIdItem(object):
    ...
    def show(self):
        ...
        bytecode._PrintDefault('class_idx=%d proto_idx=%d name_idx=%d\n'
                                % (self.class_idx, self.proto_idx,
                               self.name_idx))
        bytecode._PrintDefault('class_idx_value=%s proto_idx_value=%s name_idx_value=%s\n'
                                % (self.class_idx_value,
                               self.proto_idx_value,
                               self.name_idx_value))

    def get_obj(self):
        return pack('H', self.class_idx) + pack('H', self.proto_idx) \
            + pack('I', self.name_idx)
...
class EncodedField(object):
    ...
    def get_raw(self):
        return writeuleb128(self.field_idx_diff) \
            + writeuleb128(self.access_flags)
    ...
    def pretty_show(self):
        ...
        bytecode._PrintDefault('%s->%s %s [access_flags=%s]\n'
                               % (self.get_class_name(),
                               self.get_name(), self.get_descriptor(),
...
class ClassDataItem(object):
    ...
    def get_fields(self):
        ...
        return [x for x in self.static_fields] + [x for x in
                self.instance_fields]
    ...
    def get_obj(self):
        return [i for i in self.static_fields] + [i for i in
                self.instance_fields] + [i for i in
                self.direct_methods] + [i for i in self.virtual_methods]
...
class ClassDefItem(object):
    ...
    def get_obj(self):
        if self.interfaces_off != 0:
            self.interfaces_off = \
                self.__CM.get_obj_by_offset(self.interfaces_off).get_off()
        ...
        if self.class_data_off != 0:
            self.class_data_off = \
                self.__CM.get_obj_by_offset(self.class_data_off).get_off()

        if self.static_values_off != 0:
            self.static_values_off = \
                self.__CM.get_obj_by_offset(self.static_values_off).get_off()

        return pack('=I', self.class_idx) + pack('=I',
                self.access_flags) + pack('=I', self.superclass_idx) \
            + pack('=I', self.interfaces_off) + pack('=I',
                self.source_file_idx) + pack('=I',
                self.annotations_off) + pack('=I', self.class_data_off) \
...
class EncodedCatchHandler(object):
    ...
    def get_raw(self):
        buff = writesleb128(self.size) + ''.join(i.get_raw() for i in
                self.handlers)
...
class FillArrayData(object):
    ...
    def get_raw(self):
        return pack('=H', self.ident) + pack('=H', self.element_width) \
...
class SparseSwitch(object):
    ...
    def get_raw(self):
        return pack('=H', self.ident) + pack('=H', self.size) \
            + ''.join(pack('=l', i) for i in self.keys) \
            + ''.join(pack('=l', i) for i in self.targets)
...
class PackedSwitch(object):
    ...
    def get_output(self, idx=-1):
      ...
      """
        ...
        return ' '.join('%x' % (self.first_key + i) for i in range(0,
                        len(self.targets)))
    ...
    def get_raw(self):
        return pack('=H', self.ident) + pack('=H', self.size) \
            + pack('=i', self.first_key) + ''.join(pack('=l', i)
                for i in self.targets)
...
class Instruction35c(Instruction):
    ...
    def get_output(self, idx=-1):
        ...
        if self.A == 0:
            ...
            buff += 'v%d, v%d, v%d, v%d, %s' % (self.C, self.D, self.E,
                    self.F, kind)
        elif self.A == 5:
    ...
    def get_operands(self, idx=-1):
        ...
        if self.A == 0:
            ...
            l.extend([(OPERAND_REGISTER, self.C), (OPERAND_REGISTER,
                     self.D), (self.get_kind() + OPERAND_KIND,
                     self.BBBB, kind)])
        elif self.A == 3:
            l.extend([(OPERAND_REGISTER, self.C), (OPERAND_REGISTER,
                     self.D), (OPERAND_REGISTER, self.E),
                     (self.get_kind() + OPERAND_KIND, self.BBBB, kind)])
        elif self.A == 4:
            l.extend([(OPERAND_REGISTER, self.C), (OPERAND_REGISTER,
                     self.D), (OPERAND_REGISTER, self.E),
                     (OPERAND_REGISTER, self.F), (self.get_kind()
                     + OPERAND_KIND, self.BBBB, kind)])
        elif self.A == 5:
            l.extend([
                (OPERAND_REGISTER, self.C),
                (OPERAND_REGISTER, self.D),
                (OPERAND_REGISTER, self.E),
                (OPERAND_REGISTER, self.F),
                (OPERAND_REGISTER, self.G),
                (self.get_kind() + OPERAND_KIND, self.BBBB, kind),
                ])
    ...
    def get_raw(self):
        return pack('=HHH', self.A << 0x0c | self.G << 8 | self.OP,
                    self.BBBB, self.F << 0x0c | self.E << 8 | self.D
                    << 4 | self.C)
...
class Instruction21h(Instruction):
    ...
    def __init__(self, cm, buff):
      ...
      # log_andro.debug("OP:%x %s AA:%x BBBBB:%x" % (self.OP, args[0], self.AA, self.BBBB))
        ...
        if self.OP == 0x15:
            self.formatted_operands.append(unpack('=f', '\x00\x00'
                    + pack('=h', self.BBBB))[0])
        elif self.OP == 0x19:
            self.formatted_operands.append(unpack('=d',
                    '\x00\x00\x00\x00\x00\x00' + pack('=h',
                    self.BBBB))[0])
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AA), (OPERAND_LITERAL,
                self.BBBB)]
...
class Instruction11n(Instruction):
    ...
    def get_raw(self):
        return pack('=h', self.B << 0x0c | self.A << 8 | self.OP)
...
class Instruction21c(Instruction):
    ...
    def get_operands(self, idx=-1):
        kind = get_kind(self.cm, self.get_kind(), self.BBBB)
        return [(OPERAND_REGISTER, self.AA), (self.get_kind()
                + OPERAND_KIND, self.BBBB, kind)]
...
class Instruction22c(Instruction):
    ...
    def get_raw(self):
        return pack('=HH', self.B << 0x0c | self.A << 8 | self.OP,
                    self.CCCC)
...
class Instruction31c(Instruction):
    ...
    def get_operands(self, idx=-1):
        kind = get_kind(self.cm, self.get_kind(), self.BBBBBBBB)
        return [(OPERAND_REGISTER, self.AA), (self.get_kind()
                + OPERAND_KIND, self.BBBBBBBB, kind)]
...
class Instruction51l(Instruction):
    ...
    def __init__(self, cm, buff):
        ...
        self.BBBBBBBBBBBBBBBB = unpack('=q', buff[2:0x0a])[0]

        self.formatted_operands = []

        if self.OP == 0x18:
            self.formatted_operands.append(unpack('=d', pack('=q',
                    self.BBBBBBBBBBBBBBBB))[0])
    ...
    def get_raw(self):
        return pack('=Hq', self.AA << 8 | self.OP,
                    self.BBBBBBBBBBBBBBBB)
...
class Instruction22x(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AA), (OPERAND_REGISTER,
                self.BBBB)]
...
class Instruction23x(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AA), (OPERAND_REGISTER,
                self.BB), (OPERAND_REGISTER, self.CC)]

    def get_raw(self):
        return pack('=HH', self.AA << 8 | self.OP, self.CC << 8
                    | self.BB)
...
class Instruction21t(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AA), (OPERAND_OFFSET,
                self.BBBB)]
...
class Instruction22b(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AA), (OPERAND_REGISTER,
                self.BB), (OPERAND_LITERAL, self.CC)]
...
class Instruction3rc(Instruction):
    ...
    def get_operands(self, idx=-1):
        ...
        if self.CCCC == self.NNNN:
            return [(OPERAND_REGISTER, self.CCCC), (self.get_kind()
                    + OPERAND_KIND, self.BBBB, kind)]
        else:
            l = []
    ...
    def get_raw(self):
        return pack('=HHH', self.AA << 8 | self.OP, self.BBBB,
                    self.CCCC)
...
class Instruction32x(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_REGISTER, self.AAAA), (OPERAND_REGISTER,
                self.BBBB)]
...
class Instruction20bc(Instruction):
    ...
    def get_operands(self, idx=-1):
        return [(OPERAND_LITERAL, self.AA), (OPERAND_LITERAL,
                self.BBBB)]
...
class Instruction41c(Instruction):
    ...
    def get_operands(self, idx=-1):
        kind = get_kind(self.cm, self.get_kind(), self.BBBBBBBB)
        return [(OPERAND_REGISTER, self.AAAA), (self.get_kind()
                + OPERAND_KIND, self.BBBBBBBB, kind)]
...
class Instruction40sc(Instruction):
    ...
    def get_operands(self, idx=-1):
        kind = get_kind(self.cm, self.get_kind(), self.BBBBBBBB)
        return [(OPERAND_LITERAL, self.AAAA), (self.get_kind()
                + OPERAND_KIND, self.BBBBBBBB, kind)]
...
class Instruction52c(Instruction):
    ...
    def __init__(self, cm, buff):
        ...
        self.AAAA = unpack('=H', buff[6:8])[0]
        self.BBBB = unpack('=H', buff[8:0x0a])[0]
    ...
    def get_operands(self, idx=-1):
        kind = get_kind(self.cm, self.get_kind(), self.CCCCCCCC)
        return [(OPERAND_LITERAL, self.AAAA), (OPERAND_LITERAL,
                self.BBBB), (self.get_kind() + OPERAND_KIND,
                self.CCCCCCCC, kind)]
    ...
    def get_raw(self):
        return pack('=HIHH', self.OP, self.CCCCCCCC, self.AAAA,
                    self.BBBB)
...
class Instruction5rc(Instruction):
    ...
    def __init__(self, cm, buff):
        ...
        self.BBBBBBBB = unpack('=I', buff[2:6])[0]
        self.AAAA = unpack('=H', buff[6:8])[0]
        self.CCCC = unpack('=H', buff[8:0x0a])[0]
    ...
    def get_raw(self):
        return pack('=HIHH', self.OP, self.BBBBBBBB, self.AAAA,
                    self.CCCC)


DALVIK_OPCODES_FORMAT = {  # unused
                           # unused
                           # unused
                           # expanded opcodes
    0: [Instruction10x, ['nop']],
    1: [Instruction12x, ['move']],
    2: [Instruction22x, ['move/from16']],
    3: [Instruction32x, ['move/16']],
    4: [Instruction12x, ['move-wide']],
    5: [Instruction22x, ['move-wide/from16']],
    6: [Instruction32x, ['move-wide/16']],
    0x07: [Instruction12x, ['move-object']],
    8: [Instruction22x, ['move-object/from16']],
    ...
    0xf9: [Instruction3rms, ['invoke-virtual-quick/range',
           VTABLE_OFFSET]],
    0xfa: [Instruction35ms, ['invoke-super-quick', VTABLE_OFFSET]],
    0xfb: [Instruction3rms, ['invoke-super-quick/range',
           VTABLE_OFFSET]],
...
DALVIK_OPCODES_EXTENDED_WIDTH = {
    255: [Instruction41c, ['const-class/jumbo', KIND_TYPE]],
...
class TryItem(object):
    ...
    def get_raw(self):
        return pack('=I', self.start_addr) + pack('=H',
                self.insn_count) + pack('=H', self.handler_off)
...
class DalvikCode(object):
    ...
    def __init__(self, buff, cm):
        ...
        self.code = DCode(self.__CM, buff.get_idx(), self.insns_size,
    ...
    def get_raw(self):
        ...
        buff += pack('=H', self.registers_size) + pack('=H',
                self.ins_size) + pack('=H', self.outs_size) + pack('=H'
                , self.tries_size) + pack('=I', self.debug_info_off) \
            + pack('=I', self.insns_size) + code_raw
    ...
    def get_size(self):
        ...
        length += len(pack('=H', self.registers_size) + pack('=H',
                      self.ins_size) + pack('=H', self.outs_size)
                      + pack('=H', self.tries_size) + pack('=I',
                      self.debug_info_off) + pack('=I',
                      self.insns_size))
...
class ClassManager(object):
    ...
    def get_field(self, idx):
        ...
        return [field.get_class_name(), field.get_type(),
                field.get_name()]
    ...
    def set_hook_class_name(self, class_def, value):
        python_export = True
        _type = self.__manage_item['TYPE_TYPE_ID_ITEM'
                                   ].get(class_def.get_class_idx())
    ...
    def set_hook_method_name(self, encoded_method, value):
        ...
        method = self.__manage_item['TYPE_METHOD_ID_ITEM'
                                    ].get(encoded_method.get_method_idx())
        ...
        if class_def is not None:
            try:
                ...
                name += '_' \
                    + bytecode.FormatDescriptorToPython(encoded_method.get_descriptor())
            ...
            # python_export = False
                ...
                try:
                    name = 'METHOD_' \
                        + bytecode.FormatNameToPython(encoded_method.get_name()
                            + '_' + encoded_method.proto.replace(' ', ''
                            ).replace('(', '').replace('[', ''
                            ).replace(')', '').replace('/', '_'
                            ).replace(';', ''))
                except AttributeError:
    ...
    def set_hook_field_name(self, encoded_field, value):
        ...
        field = self.__manage_item['TYPE_FIELD_ID_ITEM'
                                   ].get(encoded_field.get_field_idx())
        ...
        if class_def is not None:
            try:
                ...
                name += '_' \
                    + bytecode.FormatDescriptorToPython(encoded_field.get_descriptor())
...
class MapList(object):
    ...
    def get_raw(self):
        return pack('=I', self.size) + ''.join(x.get_raw() for x in
                self.map_item)
...
class DalvikVMFormat(bytecode._Bytecode):
                ...
                for i in gvm.GI.successors(gvm.nodes[key].id):
                    ...
                    if python_export == True:
                        name = \
                            ...
                            + '__' \
                            + bytecode.FormatNameToPython(xref_meth.get_name()) \
                            + '__' \
                            + bytecode.FormatDescriptorToPython(xref_meth.get_descriptor())
                        setattr(_class.XREFfrom, name, xref_meth)
                    _class.XREFfrom.add(xref_meth,
                            xref.edges[gvm.nodes[key]])
                ...
                if key in gvm.nodes:
                    for i in gvm.G.predecessors(gvm.nodes[key].id):
                        ...
                        if xref_meth != None:
                            ...
                            if python_export == True:
                                setattr(method.XREFfrom, name,
                                        xref_meth)
                            method.XREFfrom.add(xref_meth,
                                    xref.edges[gvm.nodes[key]])
                    ...
                    for i in gvm.G.successors(gvm.nodes[key].id):
                        ...
                        if xref_meth != None:
                            ...
                            method.XREFto.add(xref_meth,
                                    gvm.nodes[key].edges[xref])
    ...
    def create_dref(self, python_export=True):
        ...
        for _class in self.get_classes():
            for field in _class.get_fields():
                ...
                paths = \
                    vmx.tainted_variables.get_field(field.get_class_name(),
                        field.get_name(), field.get_descriptor())
                ...
                if paths != None:
                    ...
                    for path in paths.get_paths():
                        ...
                        if access_val == 'R':
                            ...
                            name = \
                                bytecode.FormatClassToPython(dref_meth.get_class_name()) \
                                + '__' \
                                + bytecode.FormatNameToPython(dref_meth.get_name()) \
                                + '__' \
    ...
    def _create_python_export_class(self, _class, delete=False):
        if _class != None:
            ...
            name = 'CLASS_' \
                + bytecode.FormatClassToPython(_class.get_name())
            if delete:
                delattr(self, name)
                return
            else:
    ...
    def disassemble(self, offset, size):
      ...
      """
        ...
        for i in DCode(self.CM, offset, size,
                       self.get_buff()[offset:offset
                       + size]).get_instructions():
            yield i
    ...
    def colorize_operands(self, operands, colors):
        for operand in operands:
            if operand[0] == OPERAND_REGISTER:
                ...
                yield '%s%s%s' % (colors['raw'], operand[1],
                                  colors['normal'])
            elif operand[0] == OPERAND_OFFSET:

                yield '%s%d%s' % (colors['offset'], operand[1],
                                  colors['normal'])
    ...
    def get_operand_html(
        ...
        if operand[0] == OPERAND_REGISTER:
            ...
            if len(operand[1]) > 0x20:
                wrapped = wrap_fct(operand[1], 0x20)
                wrapped_adjust = '<br/>' \
                    + '<br/>'.join(escape_fct(repr(i)[1:-1]) for i in
                                   wrapped)
                return '<FONT color="%s">%s</FONT>' % (colors['raw'],
                        wrapped_adjust)

            return '<FONT color="%s">%s</FONT>' % (colors['raw'],
                    escape_fct(repr(operand[1])[1:-1]))
        elif operand[0] == OPERAND_OFFSET:
            ...
            if operand[0] == OPERAND_KIND + KIND_STRING:
                if len(operand[2]) > 0x20:
                    wrapped = wrap_fct(operand[2], 0x20)
                    wrapped_adjust = '<br/>' \
...
class OdexHeaderItem(object):
    ...
    def get_raw(self):
        return pack('=I', self.dex_offset) + pack('=I',
                self.dex_length) + pack('=I', self.deps_offset) \
            + pack('=I', self.deps_length) + pack('=I',
                self.aux_offset) + pack('=I', self.aux_length) \
            + pack('=I', self.flags) + pack('=I', self.padding)
...
class OdexDependencies(object):
    ...
    def __init__(self, buff):
        ...
        for i in range(0, self.dependency_count):
            ...
            self.dependency_checksums.append(buff.read(0x14))
    ...
    def get_raw(self):
        ...
        for (idx, value) in enumerate(self.dependencies):
            dependencies += pack('=I', len(value)) + pack('=%ds'
                    % len(value), value) + pack('=20s',
                    self.dependency_checksums[idx])

        return pack('=I', self.modification_time) + pack('=I',
                self.crc) + pack('=I', self.dalvik_build) + pack('=I',
...
class DalvikOdexVMFormat(DalvikVMFormat):
    ...
    def _preload(self, buff):
        self.orig_buff = buff
        self.magic = buff[:8]
        if self.magic == ODEX_FILE_MAGIC_35 or self.magic \
            == ODEX_FILE_MAGIC_36:
    ...
    def save(self):
      ...
      """
        ...
        return self.magic + self.odex_header.get_raw() + dex_raw \
            + self.dependencies.get_raw() + self.padding

    def get_buff(self):
        return self.magic + self.odex_header.get_raw() \
            + super(DalvikOdexVMFormat, self).get_buff() \
            + self.dependencies.get_raw() + self.padding
...
''' \
        % (method.get_class_name(), method.get_name(),
           method.get_descriptor(), method.get_access_flags_string())
    if method.code != None:
            ...
            for ins in instructions:
                ...
                if ins == instructions[-1] and i.childs != []:
                    ...
                    if (op_value == 0x2b or op_value == 0x2c) \
                        ...
                        bb_buffer += ' '.join('%d:%s' % (values[j],
                                i.childs[j + 1][2].name) for j in
                                range(0, len(i.childs) - 1)) + ' ]'
                    else:
...
def auto(filename, raw=None):
  ...
  """
    ...
    if raw == None:
        ...
        ret_type = is_android_raw(data_raw[:0x0a])
        if ret_type == 'DEX':
